...
export default function GraphView() {
  const { graph, loading, error, setSelectedNode, cached } = useGraphStore();
  const graphRef = useRef(); // reference to the ForceGraph3D instance

  const handleNodeClick = useCallback((node) => {
    setSelectedNode(node);
    if (graphRef.current) {
      graphRef.current.cameraPosition(
        { x: node.x * 1.5, y: node.y * 1.5, z: node.z * 1.5 }, // new position
        node,                                                       // look at this point
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {/* Cached badge — shows if result came from DB */}
      {cached && (
      ...
      <div className="absolute bottom-4 left-4 z-10 flex gap-3">
        ...
        <StatBadge label="Health" value={`${graph.stats.healthScore}%`} color="text-emerald-400" />
        {graph.stats.circularDeps.length > 0 && (
          <StatBadge label="Circular" value={graph.stats.circularDeps.length} color="text-yellow-400" />
        )}
      </div>
      ...
      <ForceGraph3D
        ...
        nodeId="id"
        nodeLabel={(node) =>
            `<div style="background:#18181b;border:1px solid #3f3f46;padding:8px 12px;border-radius:8px;font-family:monospace;font-size:12px;color:white;max-width:200px">
                <div style="color:#10b981;margin-bottom:4px">${node.label}</div>
                <div style="color:#71717a">In: ${node.inDegree} · Out: ${node.outDegree} · Score: ${node.complexity}</div>
            </div>`
        }
        ...
        linkDirectionalParticleColor={() => "#10b981"} // emerald moving dots on links
      />
    </div>
  );
}
